...
def get_best_feature_and_threshold(dataset):
...
def find_split(dataset):
    best_feature, threshold = get_best_feature_and_threshold(dataset)
...
def decision_tree_learning(training_dataset, current_depth, depth_limit=None, entropy_values=[]):
    current_entropy = calculate_entropy(training_dataset[:, -1])
    ...
    l_branch, l_depth = decision_tree_learning(l_dataset, current_depth + 1, depth_limit, entropy_values)
    r_branch, r_depth = decision_tree_learning(r_dataset, current_depth + 1, depth_limit, entropy_values)
...
if __name__ == '__main__': 
    ...
    full_dataset = np.loadtxt(dataset_path)
    training_dataset, test_dataset = split_dataset(full_dataset, train_ratio=0.8)

    entropy_values = []
    node_dictionary, max_depth_achieved = decision_tree_learning(training_dataset, current_depth=0, depth_limit=None, entropy_values=entropy_values)


    print('Maximum Tree Depth: ', max_depth_achieved)

    model_accuracy = evaluate(test_dataset, node_dictionary)
    print('Current Model Accuracy: ', model_accuracy)
    
    confusion_matrix = run_cross_validation(full_dataset, folds=10)
